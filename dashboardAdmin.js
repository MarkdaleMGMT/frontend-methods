...
var balance_currency = "clam_balance" // selected currency
var currency = [{ currency: "CLAM", click: true }, { currency: "BTC", click: false }, { currency: "OZ", click: false }, { currency: "CAD", click: false }, { currency: "USD", click: false }] // handles selecting tabs

currencyData = async () => {
    let response = await getAccountsByName("admin_miner") // retrieves data of admin miner TODO: do not hardcode admin_miner, dynamically retrieve the username of the admin_miner
...
addRow = async (userData) => { // generates row in table
...
updateUserTable = async () => { // updates table
...
currentCurrency = () => { // returns the current currency
...
updateInvestment = async () => { // updates the current investment to selected dropdown
...
generateDropdown = async () => { // generates dropdown
    let sel = document.getElementById("investment-select")
    sel.innerHTML = ""
...
changePageCurrency = async (cur) => { // switch pages to another currency
...
async function main() {
    ...
    updateInvestment()
    document.getElementById("CLAM").onclick = async () => { // handles selecting CLAM tab
        for (let i = 0; i < currency.length; i++) {
    ...
    document.getElementById("BTC").onclick = async () => { // handles selecting BTC tab
        for (let i = 0; i < currency.length; i++) {
    ...
    document.getElementById("updateButton").onclick = async () => { // handles global update
        ...
        if (amount != null && amount != undefined && amount != "" && !isNaN(amount) && parseFloat(amount) >= 0) {
            ...
            if (resp_json.code == "balance updated") { // successful update
    ...
    document.getElementById("depositButton").onclick = async () => { // handles depositing
    ...
    document.getElementById("transferButton").onclick = async () => {
        ...
        let account_id = null
        let curr_balance = null
        for (let i = 0; i < accounts.length; i++) { // retrieves account to transfer from
            if (accounts[i].investment_id == sel.options[sel.selectedIndex].value) {
                account_id = accounts[i].account_id
                curr_balance = accounts[i].balance
            }
